...
if os_name == 'Windows' :
    env_name = f".../Drone_run/{game}"
elif os_name =='Darwin' :
    env_name = f"../envs/{game}_{os_name}"

date_time = datetime.datetime.now().strftime("%Y%m%d%H%M%S")
save_path = f"./saved_models/{game}/DDPG/{date_time}"
load_path = f"./saved_models/{game}/DDPG/20210514201212"
...
class DDPGAgent() :
    ...
    def appen_sample(self, state, action, reward, next_state, done) :
    ...
    def soft_update_target(self) :
        for target_param, local_param in zip(self.target_actor.parameters(),
            ...
            target_param.data.copy_(tau * local_param +(1.0 - tau) *
                                    target_param.data)
...
if __name__ == '__main__' :
    ...
    engine_configuration_channel.set_configuration_parameters(time_scale=12)
    ...
    for step in range(run_step + test_step) :
        ...
        if train_mode :
            agent.appen_sample(state[0], action[0], reward, next_state[0], [done])
